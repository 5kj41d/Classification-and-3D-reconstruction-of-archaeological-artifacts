...
def train_cifar(config, data_dir=None):
    ...
    for epoch in range(start_epoch, 10):  # loop over the dataset multiple times
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            torch.save(
                net.state_dict(), os.path.join(tmpdir, "model.pt"))
            torch.save(
                optimizer.state_dict(), os.path.join(tmpdir, "optim.pt"))
            torch.save(
                {"epoch": epoch}, os.path.join(tmpdir, "extra_state.pt"))
            train.report(
...
# Configure ray tune search space
