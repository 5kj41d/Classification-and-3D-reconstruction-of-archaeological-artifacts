...
def main():
    ...
    generated_config = {
        ...
        "path": r"C:\Users\Mate\Projects\Classifiers\saved\gen_filtered"
    }

    coins = r"C:\Users\Mate\Projects\Classifiers\data\generated_iron\5337filtered_plus_vanilla"
    viking = r"C:\Users\Mate\Downloads\viking\3558"
    others = r"C:\Users\Mate\Projects\Classifiers\data\split-others\5337others"
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(np.zeros(len(labels)), labels)):
        ...
        train_loader = DataLoader(
            dataset=fold_data, 
            batch_size=generated_config["batch_size"],
            num_workers=0,
        ...
        conv_kernel=generated_config["conv_kernel"], 
        conv_stride=generated_config["conv_stride"], 
        pool_kernel=generated_config["pool_kernel"], 
        pool_stride=generated_config["pool_stride"], 
        fc1_size=generated_config["fc1_size"], 
        fc2_size=generated_config["fc2_size"])
        
        model.to(device)

        # Define loss function and optimizer
        loss_fn = nn.CrossEntropyLoss()
        optimizer = optim.Adam(model.parameters(), lr=generated_config["lr"])

        epoch_losses = {}

        # Run the training 
        for epoch in range(generated_config["epochs"]):
            loss = train_func(model, optimizer, train_loader, loss_fn)
            epoch_losses[f"epoch_{epoch + 1}"] = loss
        
        # Run the test
        print("Training done, beginning test...")
        result = best_test(model, test_loader, fold, generated_config)
        acc = result["accuracy"]
        print(f"Accuracy: {acc:.4f}")

        # Save results and losses
        write_json(result, os.path.join(generated_config["path"], f"result{fold + 1}.json"))
        write_json(epoch_losses, os.path.join(generated_config["path"], f"losses_fold{fold + 1}.json"))

        # Save model
        print("Fold evaluated, saving model...")
        model_path = os.path.join(generated_config["path"], f"model_fold_{fold + 1}.pt")
