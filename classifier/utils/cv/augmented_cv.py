...
def main():
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(np.zeros(len(labels)), labels)):
        ...
        train_loader = DataLoader(
            dataset=fold_data, 
            batch_size=augmented_config["batch_size"],
            num_workers=0,
        ...
        conv_kernel=augmented_config["conv_kernel"], 
        conv_stride=augmented_config["conv_stride"], 
        pool_kernel=augmented_config["pool_kernel"], 
        pool_stride=augmented_config["pool_stride"], 
        fc1_size=augmented_config["fc1_size"], 
        fc2_size=augmented_config["fc2_size"])
        
        model.to(device)

        # Define loss function and optimizer
        loss_fn = nn.CrossEntropyLoss()
        optimizer = optim.Adam(model.parameters(), lr=augmented_config["lr"])

        epoch_losses = {}

        # Run the training 
        for epoch in range(augmented_config["epochs"]):
            loss = train_func(model, optimizer, train_loader, loss_fn)
            epoch_losses[f"epoch_{epoch + 1}"] = loss
        
        # Run the test
        print("Training done, beginning test...")
        result = best_test(model, test_loader, fold, augmented_config)
        acc = result["accuracy"]
        print(f"Accuracy: {acc:.4f}")

        # Save results and losses
        write_json(result, os.path.join(augmented_config["path"], f"result{fold + 1}.json"))
        write_json(epoch_losses, os.path.join(augmented_config["path"], f"losses_fold{fold + 1}.json"))

        # Save model
        print("Fold evaluated, saving model...")
        model_path = os.path.join(augmented_config["path"], f"model_fold_{fold + 1}.pt")
